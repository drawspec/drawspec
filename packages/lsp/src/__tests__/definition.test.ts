...
// Use the actual monorepo root so existsSync checks pass
const REPO_ROOT = import.meta.dir.replace(/packages\/lsp\/src\/__tests__$/, "");
...
describe("provideDefinition", () => {
  test("resolves import to package entry point", () => {
    ...
    const defs = provideDefinition("file:///tmp/test.seq.ts", text, pos(0, 32), REPO_ROOT);
  ...
  test("resolves factory function call to package entry", () => {
    ...
    const defs = provideDefinition("file:///tmp/test.seq.ts", text, pos(2, 1), REPO_ROOT);
    expect(defs.length).toBe(1);
    expect(defs[0]?.targetUri).toContain("uml-sequence");
  });
  ...
  test("returns empty for unknown position", () => {
    ...
    const defs = provideDefinition("file:///tmp/test.ts", text, pos(0, 6), REPO_ROOT);
    expect(defs).toHaveLength(0);
  });

  test("returns empty when cursor is not on import string", () => {
    const text = SEQUENCE_IMPORT;
    const defs = provideDefinition("file:///tmp/test.ts", text, pos(0, 5), REPO_ROOT);
    expect(defs).toHaveLength(0);
  });

  test("returns empty when rootPath is not provided for import", () => {
    const text = SEQUENCE_IMPORT;
    const defs = provideDefinition("file:///tmp/test.ts", text, pos(0, 32), undefined);
    expect(defs).toHaveLength(0);
  });

  test("returns empty when rootPath is not provided for factory", () => {
    const text = [SEQUENCE_IMPORT, "", 'sequence("Test", (s) => {});'].join("\n");
    const defs = provideDefinition("file:///tmp/test.ts", text, pos(2, 1), undefined);
    expect(defs).toHaveLength(0);
  });
});
